...
(bkcore.hexgl = bkcore.hexgl || {}),
  (bkcore.hexgl.ShipControls = function (t) {
    ...
    if (
      ...
      (this.airBrake = 0.02),
      (this.maxSpeed = 7),
      (this.boosterSpeed = 0.2 * this.maxSpeed),
      ...
      this.gamepadController = new bkcore.controllers.GamepadController(
        function (e) {
          e.select ? t.restart() : (i.key.forward = e.acceleration > 0),
            (i.key.ltrigger = e.ltrigger > 0),
            (i.key.rtrigger = e.rtrigger > 0),
            (i.key.left = e.lstickx < -0.1),
            (i.key.right = e.lstickx > 0.1);
        }
      );
    else if (2 == t.controlType) {
      if (null == Leap) throw Error("Unable to reach LeapJS!");
      var s = (this.leapInfo = document.getElementById("leapinfo"));
      isServerConnected = !1;
      var o = (this.leapBridge = {
        isConnected: !0,
        hasHands: !1,
        palmNormal: [0, 0, 0],
      });
      function h() {
        isServerConnected
          ? o.isConnected && o.hasHands
            ...
            : ((s.innerHTML = "Please connect your Leap Motion Controller."),
              (s.style.display = "block"))
          : ((s.innerHTML = "Waiting for the Leap Motion Controller server..."),
            (s.style.display = "block"));
      }
      h();
      var r = (this.leapController = new Leap.Controller({
        enableGestures: !1,
      }));
      r.on("connect", function () {
        (isServerConnected = !0), h();
      }),
        r.on("deviceConnected", function () {
          (o.isConnected = !0), h();
        }),
        r.on("deviceDisconnected", function () {
          (o.isConnected = !1), h();
        }),
        r.on("frame", function (t) {
          o.isConnected &&
            (void 0 === (hand = t.hands[0])
              ? (o.hasHands && ((o.hasHands = !1), h()),
                (o.palmNormal = [0, 0, 0]))
              : (o.hasHands || ((o.hasHands = !0), h()),
                (o.palmNormal = hand.palmNormal)));
        }),
        r.connect();
    }
    e.addEventListener(
  ...
  (bkcore.hexgl.ShipControls.prototype.update = function (t) {
    ...
    var i = 0,
      ...
      s = 0;
    if (
      (null != this.leapBridge &&
        this.leapBridge.hasHands &&
        ((i -= 3.5 * this.leapBridge.palmNormal[0] * this.rollAngle),
        (s = -(0.6 * this.leapBridge.palmNormal[2]))),
      this.active &&
        (null != this.touchController
          ? ((e -=
              (this.touchController.stickVector.x / 100) *
              this.angularSpeed *
              t),
          ...
          ? ((e -= this.gamepadController.lstickx * this.angularSpeed * t),
            (i += this.gamepadController.lstickx * this.rollAngle))
          : null != this.leapBridge && this.leapBridge.hasHands
          ? ((e += 2 * this.leapBridge.palmNormal[0] * this.angularSpeed * t),
            (this.speed +=
              3 *
              Math.max(0, 0.5 + this.leapBridge.palmNormal[2]) *
              this.thrust *
              t))
          : (this.key.left &&
              ((e += this.angularSpeed * t), (i -= this.rollAngle)),
            this.key.right &&
              ((e -= this.angularSpeed * t), (i += this.rollAngle))),
        this.key.forward
      ...
      this.mesh.matrix.identity();
      var o = (this.gradientTarget - (s + this.gradient)) * this.gradientLerp;
      Math.abs(o) > this.epsilon && (this.gradient += o),
      ...
      var h = (this.tiltTarget - this.tilt) * this.tiltLerp;
      Math.abs(h) > this.epsilon && (this.tilt += h),
        Math.abs(this.tilt) > this.epsilon &&
          (this.tiltAxis.set(0, 0, 1),
          this.mesh.matrix.rotateByAxis(this.tiltAxis, this.tilt));
      var r = (i - this.roll) * this.rollLerp;
      Math.abs(r) > this.epsilon && (this.roll += r),
  ...
  (bkcore.hexgl.ShipControls.prototype.teleport = function (t, i) {
    if (
      ...
      this.mesh.matrix.identity();
      var e = (this.gradientTarget - this.gradient) * this.gradientLerp;
      Math.abs(e) > this.epsilon && (this.gradient += e),
        Math.abs(this.gradient) > this.epsilon &&
          (this.gradientAxis.set(1, 0, 0),
          this.mesh.matrix.rotateByAxis(this.gradientAxis, this.gradient));
      var s = (this.tiltTarget - this.tilt) * this.tiltLerp;
      Math.abs(s) > this.epsilon && (this.tilt += s),
        Math.abs(this.tilt) > this.epsilon &&
          (this.tiltAxis.set(0, 0, 1),
          this.mesh.matrix.rotateByAxis(this.tiltAxis, this.tilt)),
        this.mesh.applyMatrix(this.dummy.matrix),
  ...
  (bkcore.hexgl.ShipControls.prototype.boosterCheck = function (t) {
    ...
    (this.boost -= this.boosterDecay * t),
      this.boost < 0 && ((this.boost = 0), bkcore.Audio.stop("boost"));
    var i = Math.round(
    ...
    255 == s.r &&
      s.g < 127 &&
      s.b < 127 &&
      (bkcore.Audio.play("boost"), (this.boost = this.boosterSpeed)),
